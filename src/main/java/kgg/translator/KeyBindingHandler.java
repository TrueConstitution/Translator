...
public class KeyBindingHandler {
    public static final KeyBinding TRANSLATE_WORLD_KEY = new KeyBinding("key.translator.translate_world", GLFW.GLFW_KEY_U, "key.categories.translator");
    ...
    public static void register() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (TRANSLATE_SCREEN_KEY.wasPressed()) {
                Options.setAutoScreenText(!Options.isAutoScreenText());
                client.player.sendMessage(Text.literal("已%s屏幕翻译".formatted(Options.isAutoScreenText() ? "开启" : "关闭")));
            }

            while (TRANSLATE_WORLD_KEY.wasPressed()) {
                Options.setAutoWorldText(!Options.isAutoWorldText());
                client.player.sendMessage(Text.literal("已%s世界翻译".formatted(Options.isAutoWorldText() ? "开启" : "关闭")));
            }
